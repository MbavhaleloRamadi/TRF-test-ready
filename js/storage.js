...
const Storage = {
     ...
     * Compresses image and converts to base64 for Firestore storage
     * 
     * @param {File} file - File to upload
     * @param {string} reference - Payment reference (for logging)
     * @param {Function} onProgress - Progress callback (0-100)
     * @returns {Promise<string>} Base64 data URL
     * @throws {Error} If file invalid or compression fails
     */
    async uploadProof(file, reference, onProgress = null) {
        try {
            ...
            console.log(`📁 Storage: Processing ${file.name} (${this.formatFileSize(file.size)})`);

            // Step 2: Compress image progressively until it fits (10-70%)
            ...
            while (attempt < maxAttempts) {
                ...
                if (onProgress) {
                    onProgress(10 + (attempt * 12)); // 22, 34, 46, 58, 70
                }

                try {
                    // Compress the image
                    processedFile = await this.compressImage(file, maxWidth, quality);
                    console.log(`   Attempt ${attempt}: ${maxWidth}px @ ${Math.round(quality * 100)}% = ${this.formatFileSize(processedFile.size)}`);
                    ...
                    if (base64.length <= this.MAX_BASE64_SIZE) {
                        ...
                        console.log(`   ✅ Final size: ${this.formatFileSize(base64.length)} base64`);
                        
                        if (onProgress) onProgress(100);
                        return base64;
                    }
                    
                    // Still too large - reduce quality and/or size
                    if (quality > 0.3) {
                        quality -= 0.15;
                    } else if (maxWidth > this.MIN_WIDTH) {
                        maxWidth = Math.max(this.MIN_WIDTH, Math.round(maxWidth * 0.7));
                        quality = 0.6; // Reset quality for new size
                    } else {
                    ...
                    console.warn(`   Compression attempt ${attempt} failed:`, compressError.message);
            ...
            // If we get here, compression failed to meet target
            throw new Error(
        ...
        if (fileType && !this.ALLOWED_TYPES.includes(fileType)) {
            return { 
                ...
                error: `Invalid file type "${fileType}". ` +
                       `Please upload an image (JPG, PNG, GIF, or WebP).`
            };
        }

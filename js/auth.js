...
const Auth = {
    ...
    async verifyAdminCode(code) {
        try {
            ...
            // Get admin code from Firestore settings
            const settingsDoc = await db.collection('settings').doc('admin').get();
            
            if (!settingsDoc.exists) {
                // First time setup - create default admin code
            ...
            const storedCode = settingsDoc.data().adminCode;
            const isValid = code === storedCode;

            if (isValid) {
                // Create admin session
                this.setAdminSession();
                await this.logAdminAction('admin_login', { success: true });
            } else {
                await this.logAdminAction('admin_login_failed', { success: false });
            }
    ...
    async lookupMember(name, phone) {
        try {
            ...
            if (name) {
                ...
                const storedName = (memberData.fullName || memberData.name || '').toLowerCase().trim();
